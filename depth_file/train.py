...
from losses import combined_depth_loss
...
depth_model = depth_model(res_model.output)
...
loss = combined_depth_loss(y_true, y_pred)
...
#Compiling model
depth_model.compile(optimizer = optimizer, loss = combined_semantic_losses)

#Training starts from here
number_of_epoch = int(input("Enter number of epochs : "))
depth_model.fit(train_data, number_of_epoch, callbacks = callbacks, verbose=1)
